...
function issue(
  ...
  return {
    id: [severity, code, screenId, buttonId, buttonKind, JSON.stringify(params ?? {})].filter(Boolean).join(":"),
    severity,
    code,
    params,
    screenId,
...
export function isValidBotCommand(value: string): boolean {
  return /^[a-z0-9_]{1,32}$/.test(normalizeCommand(value));
}
...
export function validateProject(project: Project): ValidationIssue[] {
  ...
  for (const screen of project.screens) {
    ...
    const replyCount = screen.replyKeyboard.mode === "show"
      ? screen.replyKeyboard.config.rows.reduce((sum, row) => sum + row.buttons.length, 0)
      : 0;
      ...
      for (const button of row.buttons) {
        ...
        if (button.action.type === "callback") {
          ...
          if (bytes < TELEGRAM_CALLBACK_DATA_MIN_BYTES || bytes > TELEGRAM_CALLBACK_DATA_MAX_BYTES) {
            issues.push(issue("error", "INVALID_CALLBACK_LENGTH", { button: label, bytes, max: 64 }, screen.id, button.id, "inline"));
          }
          if (button.action.callbackData.startsWith(GENERATED_CALLBACK_PREFIX)) {
            issues.push(issue("error", "RESERVED_CALLBACK_PREFIX", { button: label, prefix: GENERATED_CALLBACK_PREFIX }, screen.id, button.id, "inline"));
          }
        }

        if (button.action.type === "url" && !isValidHttpUrl(button.action.url)) {
          issues.push(issue("error", "INVALID_URL", { button: label }, screen.id, button.id, "inline"));
        }
      }
    }
    ...
    if (screen.replyKeyboard.mode === "show") {
      ...
      for (const row of config.rows) {
        for (const button of row.buttons) {
          ...
          if (button.action.type === "webApp" && !isValidHttpsUrl(button.action.url)) {
            issues.push(issue("error", "INVALID_WEBAPP_URL", { button: label }, screen.id, button.id, "reply"));
          }
        }
      }
    }
  ...
  for (const command of project.botSettings.commands) {
    ...
    if (!command.description.trim() || characterLength(command.description) > 256) {
      issues.push(issue("error", "INVALID_BOT_COMMAND_DESCRIPTION", { command: normalized || command.command }));
    }
    if (normalized) botCommandOwners.set(normalized, [...(botCommandOwners.get(normalized) ?? []), command.id]);
  }

  for (const [command, ids] of botCommandOwners) {
    if (ids.length > 1) issues.push(issue("error", "DUPLICATE_BOT_COMMAND", { command }));
  }
  ...
  for (const screen of project.screens) {
    ...
    const replyTargets = screen.replyKeyboard.mode === "show"
      ? screen.replyKeyboard.config.rows.flatMap((row) => row.buttons.flatMap((button) => button.action.type === "screen" && screenIds.has(button.action.screenId) ? [button.action.screenId] : []))
      : [];
