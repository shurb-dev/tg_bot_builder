...
export type TelegramMessage = {
  ...
  media: null | { type: "photo"; url: string };
};
...
export type ProjectVariable = {
  ...
  defaultValue: ProjectVariableValue;
};
...
export type LogicNodeBase = {
  id: string;
  name: string;
  editor: { flowPosition: FlowPosition };
};

export type InputNode = LogicNodeBase & {
  type: "input";
  prompt: string;
  variable: string;
  inputType: InputType;
...
export type ConditionNode = LogicNodeBase & {
  type: "condition";
  combinator: "and" | "or";
  rules: ConditionRule[];
  trueTarget: FlowTarget | null;
  falseTarget: FlowTarget | null;
};

export type SetVariableNode = LogicNodeBase & {
  type: "setVariable";
  variable: string;
  value: string;
  next: FlowTarget | null;
...
export type HttpRequestNode = LogicNodeBase & {
  type: "http";
  method: HttpMethod;
  url: string;
  headers: KeyValueTemplate[];
  query: KeyValueTemplate[];
  body: HttpRequestBody;
  resultKey: string;
  timeoutMs: number;
  mock: HttpMock;
  successTarget: FlowTarget | null;
  errorTarget: FlowTarget | null;
};

export type SendMessageNode = LogicNodeBase & {
  type: "sendMessage";
  text: string;
  parseMode: "none" | "HTML" | "MarkdownV2";
  next: FlowTarget | null;
