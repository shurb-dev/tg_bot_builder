...
export function EditorShell() {
  ...
  useEffect(() => {
    const listener = (event: KeyboardEvent) => {
      ...
      if (!typing && (event.key === "Delete" || event.key === "Backspace")) {
        ...
        } else if (selection?.type === "replyButton") {
          event.preventDefault();
          removeReplyButton(selection.screenId, selection.rowId, selection.buttonId);
        }
      }
    };
  ...
  function exportJson() {
    try {
      downloadBlob(new Blob([exportProjectJson(project)], { type: "application/json;charset=utf-8" }), projectFilename(project.name));
      setToast(t.toast.jsonExported);
    } catch {
      setToast(t.toast.jsonExportFailed);
    }
  }
  ...
  async function importFile(file: File) {
    try {
      ...
      const count = importedIssues.filter((item) => item.severity === "error").length;
      if (hasBlockingErrors(importedIssues)) {
        setToast(formatTemplate(t.toast.importBlocked, { count }));
        return;
      }
      if (!window.confirm(formatTemplate(t.confirm.replace, { name: imported.name }))) return;
      replaceProject(imported);
      setToast(t.toast.projectImported);
    } catch {
  ...
  async function exportZip() {
    ...
    try {
      const blob = await buildProjectZip(generateAiogramProject(project));
      downloadBlob(blob, `${slug(project.name)}-aiogram.zip`);
      setToast(t.toast.zipExported);
    } catch {
      setToast(t.toast.zipFailed);
    }
  }

  if (hydrationStatus === "pending") {
    return <div className="grid h-screen place-items-center bg-slate-950 text-sm text-slate-500">{t.shell.loading}</div>;
  }
