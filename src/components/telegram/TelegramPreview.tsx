...
export function TelegramPreview({ screen, onSelectInlineButton, onSelectReplyButton, selectedInlineButtonId, selectedReplyButtonId }: {
  ...
  const t = useTranslations(); const [failedMediaUrl, setFailedMediaUrl] = useState<string | null>(null);
  const mediaUrl = screen.message.media?.url ?? ""; const imageFailed = Boolean(mediaUrl && failedMediaUrl === mediaUrl);
  const reply = screen.replyKeyboard.mode === "show" ? screen.replyKeyboard.config : null;
  return <div className="mx-auto flex h-[650px] w-full max-w-[420px] flex-col overflow-hidden rounded-[30px] border border-slate-700 bg-[#0d1822] shadow-2xl shadow-black/30">
    <div className="flex h-16 items-center gap-3 border-b border-white/10 bg-[#172635] px-5"><div className="grid h-9 w-9 place-items-center rounded-full bg-sky-500 text-sm font-bold text-white">TB</div><div><div className="text-sm font-semibold text-white">{screen.name || "Telegram Bot"}</div><div className="text-xs text-sky-300">{t.preview.bot}</div></div></div>
    <div className="telegram-wallpaper flex min-h-0 flex-1 flex-col justify-end overflow-y-auto p-4"><div className="w-full max-w-[92%] rounded-2xl rounded-bl-md bg-[#182a38] p-2 shadow-lg">
      ...
      {screen.inlineKeyboard.length > 0 ? <div className="mt-2 space-y-1.5" data-testid="inline-keyboard">{screen.inlineKeyboard.map((row) => <div key={row.id} className="flex gap-1.5">{row.buttons.map((button) => { const title = button.action.type === "screen" ? button.text : button.action.type === "node" ? button.text : button.action.type === "url" ? button.action.url : button.action.callbackData; return <button key={button.id} onClick={() => onSelectInlineButton(row.id, button.id)} className={`min-w-0 flex-1 rounded-lg border px-2 py-2 text-center text-xs font-medium text-sky-300 ${selectedInlineButtonId === button.id ? "border-sky-400 bg-sky-400/15 ring-1 ring-sky-400/40" : "border-sky-800/80 bg-[#20384b] hover:bg-[#29465d]"}`} title={title}><span className="block truncate">{button.text}</span></button>; })}</div>)}</div> : null}
    </div></div>
